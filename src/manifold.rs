...
impl Manifold {
    ...
    pub fn get_mesh_gl(&self, normal_idx: i32) -> MeshGL {
        ...
        let num_prop_total = 3 + extra_prop as u32;
        out.num_prop = num_prop_total;
        ...
        // Build faceID and triVerts in sorted order, tracking run boundaries
        ...
        for new_tri in 0..num_tri {
            ...
            // triVerts — use start_vert (geometry vertex), not prop_vert
            for i in 0..3 {
            ...
            if mesh_id != last_mesh_id {
                let rel = mesh_id_transform.remove(&mesh_id)
                    .unwrap_or_default();
                out.run_index.push(3 * new_tri as u32);
                out.run_original_id.push(rel.original_id.max(0) as u32);
                // Store 3x4 transform (column-major: 4 columns × 3 rows)
                for col in 0..4 {
                    for row in 0..3 {
