...
fn test_cpp_sdf_bounds() {
    ...
    assert_eq!(
        cube_void.genus(),
        -1,
        "SDF CubeVoid genus should be -1, got {}",
        cube_void.genus()
    );
    ...
    assert!(
        (bounds.min.x - (-outer_bound)).abs() < epsilon + 0.1,
        "min.x: {} expected ~{}",
        bounds.min.x,
        -outer_bound
    );
    assert!(
        (bounds.max.x - outer_bound).abs() < epsilon + 0.1,
        "max.x: {} expected ~{}",
        bounds.max.x,
        outer_bound
    );
...
fn test_cpp_sdf_sphere_bounds() {
    ...
    assert_eq!(
        sphere.genus(),
        0,
        "SDF sphere genus should be 0, got {}",
        sphere.genus()
    );
    ...
    assert!(
        (bounds.min.x - (-1.0)).abs() < epsilon + 0.1,
        "min.x: {} expected ~-1",
        bounds.min.x
    );
    assert!(
        (bounds.max.x - 1.0).abs() < epsilon + 0.1,
        "max.x: {} expected ~1",
        bounds.max.x
    );
}
...
fn test_cpp_hull_empty() {
    ...
    assert!(
        h.is_empty() || h.volume().abs() < 1e-10,
        "Hull of 3 points should be empty/degenerate"
    );
    ...
    assert!(
        h2.is_empty() || h2.volume().abs() < 1e-10,
        "Hull of coplanar points should be empty/degenerate"
    );
}
...
fn test_cpp_properties_tolerance() {
    ...
    let imperfect = cube
        .intersection(&cube.rotate(degrees, 0.0, 0.0))
        .as_original();
    assert_eq!(
        imperfect.num_tri(),
        28,
        "Tolerance imperfect: {} tris expected 28",
        imperfect.num_tri()
    );
    ...
    assert_eq!(
        imperfect2.num_tri(),
        12,
        "Tolerance simplified: {} tris expected 12",
        imperfect2.num_tri()
    );

    assert!(
        (imperfect.volume() - imperfect2.volume()).abs() < 0.01,
        "Tolerance volumes: {} vs {}",
        imperfect.volume(),
        imperfect2.volume()
    );
    assert!(
        (imperfect.surface_area() - imperfect2.surface_area()).abs() < 0.02,
        "Tolerance areas: {} vs {}",
        imperfect.surface_area(),
        imperfect2.surface_area()
    );
}
...
fn test_cpp_properties_tolerance_sphere() {
    ...
    assert!(
        sphere2.num_tri() < 2500,
        "ToleranceSphere: {} tris expected < 2500",
        sphere2.num_tri()
    );
...
fn test_cpp_properties_min_gap_transformed() {
    ...
    assert!(
        ...
        "MinGapTransformed: {} expected ~1.0",
        distance
    );
}
...
fn test_cpp_properties_min_gap_transformed_oob() {
    ...
    assert!(
        ...
        "MinGapTransformedOOB: {} expected ~0.95",
        distance
    );
}
...
fn test_cpp_triangle_distance_vertices() {
    ...
    assert!(
        ...
        "TriangleDistanceVertices: {} expected 1.0",
        distance
    );
}
...
fn test_cpp_triangle_distance_edge() {
    ...
    assert!(
        (distance - 1.0).abs() < 1e-6,
        "TriangleDistanceEdge: {} expected 1.0",
        distance
    );
}
...
fn test_cpp_triangle_distance_edge2() {
    ...
    assert!(
        ...
        "TriangleDistanceEdge2: {} expected 0.5",
        distance
    );
}
...
fn test_cpp_triangle_distance_face() {
    ...
    assert!(
        (distance - 1.0).abs() < 1e-6,
        "TriangleDistanceFace: {} expected 1.0",
        distance
    );
}
...
fn test_cpp_triangle_distance_overlapping() {
    ...
    assert!(
        ...
        "TriangleDistanceOverlapping: {} expected 0.0",
        distance
    );
}
...
fn test_cpp_tree_transforms() {
    ...
    assert!(
        ...
        "TreeTransforms volume: {} expected 2.0",
        result.volume()
    );
}
...
fn test_cpp_corner_union() {
    ...
    assert_eq!(
        cubes.num_vert(),
        16,
        "CornerUnion verts: {} expected 16",
        cubes.num_vert()
    );
    assert_eq!(
        cubes.num_tri(),
        24,
        "CornerUnion tris: {} expected 24",
        cubes.num_tri()
    );
}
...
fn test_cpp_perturb1() {
    ...
    assert_eq!(
        result.num_degenerate_tris(),
        0,
        "Perturb1: has degenerate tris"
    );
    assert_eq!(
        result.num_vert(),
        24,
        "Perturb1 verts: {} expected 24",
        result.num_vert()
    );
    assert!(
        (result.volume() - 7.5).abs() < 1e-4,
        "Perturb1 volume: {} expected 7.5",
        result.volume()
    );
    assert!(
        (result.surface_area() - 38.2).abs() < 0.1,
        "Perturb1 SA: {} expected ~38.2",
        result.surface_area()
    );
}
...
fn test_cpp_cross_section_mirror_union() {
    ...
    assert!(
        ...
        "MirrorUnion area: {} expected {}",
        cross.area(),
        2.5 * a.area()
    );
    assert!(
        a.mirror(Vec2::new(0.0, 0.0)).is_empty(),
        "Mirror with zero axis should be empty"
    );
}
...
fn test_cpp_cross_section_round_offset() {
    ...
    assert_eq!(
        result.genus(),
        0,
        "RoundOffset genus: {} expected 0",
        result.genus()
    );
    assert!(
        (result.volume() - 4386.0).abs() < 50.0,
        "RoundOffset volume: {} expected ~4386",
        result.volume()
    );
}
...
fn test_cpp_cross_section_decompose() {
    ...
    assert_eq!(
        decomp.len(),
        2,
        "Decompose should produce 2 components, got {}",
        decomp.len()
    );
    assert_eq!(
        decomp[0].num_contour(),
        2,
        "Component 0 should have 2 contours, got {}",
        decomp[0].num_contour()
    );
    assert_eq!(
        decomp[1].num_contour(),
        2,
        "Component 1 should have 2 contours, got {}",
        decomp[1].num_contour()
    );
}
...
fn test_cpp_cross_section_mirror_check_axis() {
    ...
    assert!(
        (a.min.x - a_expected.min.x).abs() < 0.001,
        "MirrorCheckAxis a: min.x {} vs {}",
        a.min.x,
        a_expected.min.x
    );
    assert!(
        (a.min.y - a_expected.min.y).abs() < 0.001,
        "MirrorCheckAxis a: min.y {} vs {}",
        a.min.y,
        a_expected.min.y
    );
    assert!(
        (a.max.x - a_expected.max.x).abs() < 0.001,
        "MirrorCheckAxis a: max.x {} vs {}",
        a.max.x,
        a_expected.max.x
    );
    assert!(
        (a.max.y - a_expected.max.y).abs() < 0.001,
        "MirrorCheckAxis a: max.y {} vs {}",
        a.max.y,
        a_expected.max.y
    );
    ...
    assert!(
        (b.min.x - b_expected.min.x).abs() < 0.001,
        "MirrorCheckAxis b: min.x {} vs {}",
        b.min.x,
        b_expected.min.x
    );
    assert!(
        (b.min.y - b_expected.min.y).abs() < 0.001,
        "MirrorCheckAxis b: min.y {} vs {}",
        b.min.y,
        b_expected.min.y
    );
    assert!(
        (b.max.x - b_expected.max.x).abs() < 0.001,
        "MirrorCheckAxis b: max.x {} vs {}",
        b.max.x,
        b_expected.max.x
    );
    assert!(
        (b.max.y - b_expected.max.y).abs() < 0.001,
        "MirrorCheckAxis b: max.y {} vs {}",
        b.max.y,
        b_expected.max.y
    );
}
...
fn test_cpp_cross_section_rect() {
    ...
    assert!(
        (area - cross.area()).abs() < 1e-6,
        "Rect area {} != CrossSection area {}",
        area,
        cross.area()
    );
    assert!(
        rect.contains_point(Vec2::new(5.0, 5.0)),
        "Rect should contain (5,5)"
    );
    assert!(
        rect.contains_rect(&cross.bounds()),
        "Rect should contain cross bounds"
    );
    assert!(
        rect.contains_rect(&Rect::new()),
        "Rect should contain empty rect"
    );
    assert!(
        rect.does_overlap(&Rect::from_points(
            Vec2::new(5.0, 5.0),
            Vec2::new(15.0, 15.0)
        )),
...
fn test_cpp_cross_section_negative_offset() {
    ...
    assert!(
        (dilated.area() - expected).abs() < 1.0,
        "NegativeOffset area: {} expected {}",
        dilated.area(),
        expected
    );
}
...
fn test_cpp_boolean_edge_union() {
    ...
    assert_eq!(
        cubes.num_vert(),
        16,
        "EdgeUnion: {} verts expected 16",
        cubes.num_vert()
    );
    assert_eq!(
        cubes.num_tri(),
        24,
        "EdgeUnion: {} tris expected 24",
        cubes.num_tri()
    );
}
...
fn test_cpp_manifold_mesh_id() {
    ...
    assert!(
        !cube1.is_empty(),
        "cube1 should not be empty, status={:?}",
        cube1.status()
    );
    assert!(
        !cube2.is_empty(),
        "cube2 should not be empty, status={:?}",
        cube2.status()
    );
    let gl1 = cube1.get_mesh_gl(3);
    assert!(
        !gl1.run_original_id.is_empty(),
        "gl1 should have run_original_id"
    );
    let id1 = gl1.run_original_id[0];
    let id2 = cube2.get_mesh_gl(3).run_original_id[0];
    assert_ne!(
        id1, id2,
        "MeshID: two imports should get different IDs: {} vs {}",
        id1, id2
    );
}
