...
fn test_boolean_precision() {
    ...
    assert_eq!(
        result.num_vert(),
        8,
        "Tiny cube should be absorbed: {} verts",
        result.num_vert()
    );

    let cube3 = cube
        .scale(Vec3::splat(2.0 * scale))
        .translate(Vec3::new(distance, 0.0, 0.0));
    let result2 = result.union(&cube3);
    assert_eq!(
        result2.num_vert(),
        16,
        "2x precision cube should stay separate: {} verts",
        result2.num_vert()
    );
}
...
fn test_boolean_simple_cube_regression() {
    let result = Manifold::cube(Vec3::splat(1.0), false)
        ...
        .difference(&Manifold::cube(Vec3::splat(1.0), false).rotate(
            -0.1,
            -0.00000000000066571,
            -1.0,
        ));
...
fn test_cpp_split() {
    ...
    assert!(
        (first.volume() + second.volume() - cube.volume()).abs() < 1e-5,
...
fn test_cpp_split_by_plane() {
    ...
    assert!(
        (first.volume() - second.volume()).abs() < 1e-3,
        "Split halves should have equal volume: {} vs {}",
        first.volume(),
        second.volume()
    );
    ...
    assert!(
        (first.volume() - trimmed.volume()).abs() < 1e-3,
        "Trim should match first split: {} vs {}",
        first.volume(),
        trimmed.volume()
    );
}
...
fn test_cpp_split_by_plane60() {
    ...
    let (first, second) = cube.split_by_plane(Vec3::new(phi_rad.sin(), -phi_rad.cos(), 0.0), 1.0);
    assert!(
...
fn test_cpp_vug() {
    ...
    assert!(
        (half.volume() - (4.0 * 4.0 * 3.0 - 1.0)).abs() < 0.1,
        "volume: {} expected: {}",
        half.volume(),
        4.0 * 4.0 * 3.0 - 1.0
    );
...
fn test_cpp_winding() {
    ...
    assert!(
        !result.is_empty(),
        "Winding intersection should not be empty"
    );
}
...
fn test_cpp_batch_boolean() {
    ...
    assert!(!add.is_empty());
    assert!(
        add.volume() > cube.volume(),
        "Union volume should be >= cube volume"
    );
    ...
    assert!(
        subtract.volume() < cube.volume(),
        "Subtract volume should be < cube volume"
    );
}
...
fn test_cpp_batch_boolean_exact() {
    ...
    assert!(
        intersect.is_empty(),
        "BatchBoolean intersect should be empty"
    );
...
fn test_cpp_warp() {
    ...
    let shape =
        Manifold::extrude(&square.to_polygons(), 2.0, 10, 0.0, Vec2::new(1.0, 1.0)).warp(|v| {
            v.x += v.z * v.z;
        });
    assert!(
        (shape.volume() - 2.0).abs() < 0.0001,
        "Warped extrusion volume: {} expected: 2.0",
        shape.volume()
    );
}
...
fn test_colored_boolean_preserves_properties() {
    ...
    let a = Manifold::cube(Vec3::new(1.0, 1.0, 1.0), true).set_properties(3, |p, _, _| {
        p[0] = 0.0;
        p[1] = 0.0;
        p[2] = 1.0;
    }); // blue
    let b = Manifold::cube(Vec3::new(1.0, 1.0, 1.0), true)
        .set_properties(3, |p, _, _| {
            p[0] = 1.0;
            p[1] = 0.0;
            p[2] = 0.0;
        }) // red
...
fn test_rotate_boolean_with_properties_all_angles() {
    ...
    let a = Manifold::cube(Vec3::new(1.0, 1.0, 1.0), true).set_properties(4, |p, _, _| {
        p[0] = 0.27;
        p[1] = 0.53;
        p[2] = 0.80;
        p[3] = 1.0;
    });
    let b_base = Manifold::cube(Vec3::new(1.0, 1.0, 1.0), true).set_properties(4, |p, _, _| {
        p[0] = 0.85;
        p[1] = 0.25;
        p[2] = 0.25;
        p[3] = 0.6;
...
fn test_spiky_dodecahedron_boolean_all_angles() {
    ...
    assert!(
        a.num_tri() == 60,
        "Spiky dodecahedron should have 60 tris, got {}",
        a.num_tri()
    );
    ...
    assert!(
        result.num_tri() > 0,
        "Basic spiky dodecahedron union failed"
    );
...
fn test_cpp_union_difference() {
    let block =
        Manifold::cube(Vec3::splat(1.0), true).difference(&Manifold::cylinder(1.0, 0.5, 0.5, 32));
...
fn test_cpp_boolean_empty_ops() {
    ...
    assert!(
        (cube.union(&empty).volume() - cube_vol).abs() < 1e-10,
        "cube + empty should equal cube"
    );
    assert!(
        (cube.difference(&empty).volume() - cube_vol).abs() < 1e-10,
        "cube - empty should equal cube"
    );
    assert!(
        empty.difference(&cube).is_empty(),
        "empty - cube should be empty"
    );
    assert!(
        cube.intersection(&empty).is_empty(),
...
fn test_cpp_mirrored() {
    ...
    assert!(
        (result.volume() - 0.75).abs() < 1e-6,
        "Mirrored volume: {} expected 0.75",
        result.volume()
    );
    assert!(
        (result.surface_area() - 5.5).abs() < 1e-6,
        "Mirrored area: {} expected 5.5",
        result.surface_area()
    );
}
...
fn test_cpp_cubes_union() {
    let mut result =
        Manifold::cube(Vec3::new(1.2, 1.0, 1.0), true).translate(Vec3::new(0.0, -0.5, 0.5));
    result = result.union(
        &Manifold::cube(Vec3::new(1.0, 0.8, 0.5), false).translate(Vec3::new(-0.5, 0.0, 0.5)),
    );
    result = result.union(
        &Manifold::cube(Vec3::new(1.2, 0.1, 0.5), false).translate(Vec3::new(-0.6, -0.1, 0.0)),
    );

    assert!(
        result.matches_tri_normals(),
        "Cubes result should match tri normals"
    );
...
fn test_cpp_tetra_boolean() {
    ...
    assert!(
        result.num_tri() > 0,
        "Tetra subtraction should be non-empty"
    );
    assert!(
        result.volume() > 0.0,
        "Tetra subtraction should have positive volume"
    );
}
...
fn test_cpp_no_retained_verts() {
    ...
    assert!(
        (cube.volume() - 1.0).abs() < 0.001,
        "cube vol: {}",
        cube.volume()
    );
    assert!(
        (oct.volume() - 1.333).abs() < 0.001,
        "oct vol: {}",
        oct.volume()
    );
...
fn test_cpp_multi_coplanar() {
    ...
    assert_eq!(
        out.genus(),
        -1,
        "MultiCoplanar genus: {} expected -1",
        out.genus()
    );
    assert!(
...
fn test_cpp_properties_no_intersection() {
    ...
    let cube = Manifold::cube(Vec3::splat(1.0), false).set_properties(2, |props, pos, _old| {
        props[0] = pos.x;
        props[1] = pos.y;
    });
    ...
    assert_eq!(
        result.num_prop(),
        2,
        "PropertiesNoIntersection: num_prop should be 2, got {}",
        result.num_prop()
    );
}
...
fn test_cpp_mixed_properties() {
    let cube_uv = Manifold::cube(Vec3::splat(1.0), false).set_properties(2, |props, pos, _old| {
        props[0] = pos.x;
        props[1] = pos.y;
    });
    ...
    assert_eq!(
        result.num_prop(),
        2,
        "MixedProperties: num_prop should be 2, got {}",
        result.num_prop()
    );
}
...
fn test_operator_overloads() {
    ...
    let d = &a - &b;
    assert!(
        d.volume() > 0.0 && d.volume() < 1.0,
        "Diff volume should be in (0,1), got {}",
        d.volume()
    );
    ...
    let i = &a ^ &b;
    assert!(
        i.volume() > 0.0 && i.volume() < 1.0,
        "Intersect volume should be in (0,1), got {}",
        i.volume()
    );
...
fn test_cpp_edge_union2() {
    ...
    assert_eq!(
        result.num_vert(),
        8,
        "EdgeUnion2: {} verts expected 8",
        result.num_vert()
    );
    assert_eq!(
        result.num_tri(),
        8,
        "EdgeUnion2: {} tris expected 8",
        result.num_tri()
    );
}
...
fn test_cpp_missing_normals() {
    ...
    assert!(
        !result.is_empty(),
        "MissingNormals result should not be empty"
    );
}
...
fn test_cpp_mixed_num_prop() {
    ...
    assert_eq!(
        result.num_prop(),
        2,
        "MixedNumProp: num_prop should be 2, got {}",
        result.num_prop()
    );
}
...
fn test_cpp_create_properties_slow() {
    let a = Manifold::sphere(10.0, 1024).set_properties(3, |props, _pos, _old| {
        props[0] = 0.0;
        props[1] = 0.0;
        props[2] = 0.0;
    });
    ...
    assert_eq!(
        result.num_prop(),
        3,
        "CreatePropertiesSlow: num_prop should be 3, got {}",
        result.num_prop()
    );
}
...
fn test_cpp_simplify() {
    ...
    assert!(
        result.num_tri() > 1000,
        "Simplify: pre-simplify should have many tris, got {}",
        result.num_tri()
    );
    ...
    assert!(
        (simplified.volume() - 2.0).abs() < 0.01,
        "Simplify: volume should be 2.0, got {}",
        simplified.volume()
    );
    // Accept 12 (fully simplified box) or 20 (with partition face preserved)
    assert!(
        simplified.num_tri() == 12 || simplified.num_tri() == 20,
        "Simplify: expected 12 or 20 tris, got {}",
        simplified.num_tri()
    );
}
...
fn test_cpp_simplify_cracks() {
    ...
    assert_eq!(
        deformed.genus(),
        0,
        "SimplifyCracks: deformed genus should be 0"
    );
    assert_eq!(
        simplified.genus(),
        0,
        "SimplifyCracks: simplified genus should be 0"
    );
    assert!(
        (simplified.volume() - deformed.volume()).abs() < 10.0,
        "SimplifyCracks: volume {} vs {}",
        simplified.volume(),
        deformed.volume()
    );
    assert!(
        (simplified.surface_area() - deformed.surface_area()).abs() < 1.0,
        "SimplifyCracks: area {} vs {}",
        simplified.surface_area(),
        deformed.surface_area()
    );
}
...
fn test_cpp_mesh_gl_round_trip() {
    ...
    assert!(
        cube.original_id() >= 0,
        "MeshGLRoundTrip: cube should have original_id >= 0"
    );
    ...
    assert!(
        ...
        "MeshGLRoundTrip: union result should have negative original_id"
    );
    ...
    assert_eq!(
        ...
        2,
        "MeshGLRoundTrip: expected 2 runs"
    );
    let result2 = Manifold::from_mesh_gl(&in_gl);

    assert!(
        result2.original_id() < 0,
        "MeshGLRoundTrip: result2 should have negative original_id"
    );
    assert_eq!(
        result2.num_vert(),
        18,
        "MeshGLRoundTrip: result2 expected 18 verts"
    );
    assert_eq!(
        result2.num_tri(),
        32,
        "MeshGLRoundTrip: result2 expected 32 tris"
    );
    ...
    assert_eq!(
        ...
        2,
        "MeshGLRoundTrip: outGL expected 2 runs"
    );
}
...
fn test_cpp_normals() {
    ...
    let inner = sphere.clone().rotate(180.0, 0.0, 0.0).difference(
        &sphere
