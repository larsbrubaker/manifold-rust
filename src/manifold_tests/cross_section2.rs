...
fn test_cpp_cross_section_bevel_offset() {
    ...
    assert!(
        (result.volume() - expected_vol).abs() < 1.0,
        "BevelOffset vol={} expected~{}",
        result.volume(),
        expected_vol
    );
    assert_eq!(
...
fn test_cpp_cross_section_hull_error() {
    ...
    assert!(
        (rr.area() - 1765.179_037_5).abs() < 1.0,
        "HullError area={}",
        rr.area()
    );
...
fn test_cpp_cross_section_batch_boolean() {
    ...
    assert!(
        (intersect.area()).abs() < 1e-4,
        "BatchBoolean intersect area={}",
        intersect.area()
    );
    assert_eq!(
        intersect.num_vert(),
        0,
        "BatchBoolean intersect NumVert={}",
        intersect.num_vert()
    );

    let add = CrossSection::batch_boolean(&sections, OpType::Add);
    assert!(
        (add.area() - 16278.637_002).abs() < 1.0,
        "BatchBoolean add area={}",
        add.area()
    );
    assert_eq!(
        add.num_vert(),
        66,
        "BatchBoolean add NumVert={}",
        add.num_vert()
    );

    let subtract = CrossSection::batch_boolean(&sections, OpType::Subtract);
    assert!(
        (subtract.area() - 7234.478_452).abs() < 1.0,
        "BatchBoolean subtract area={}",
        subtract.area()
    );
    assert_eq!(
        subtract.num_vert(),
        42,
        "BatchBoolean subtract NumVert={}",
        subtract.num_vert()
    );
}
...
fn test_cpp_cross_section_rect() {
    ...
    assert!(
        (rect.area() - cross.area()).abs() < 1e-6,
        "Rect vs CrossSection area"
    );
    assert!(
...
fn test_cpp_cross_section_square() {
    ...
    assert!(
        (a.difference(&b).volume()).abs() < 1e-4,
        "Square: a-b volume should be ~0"
    );
}
...
fn test_cpp_cross_section_mirror_union() {
    ...
    assert!(
        (2.5 * a.area() - cross.area()).abs() < 1e-4,
        "MirrorUnion: cross.area={} expected ~{}",
        cross.area(),
        2.5 * a.area()
    );
    assert!(
        a.mirror(Vec2::new(0.0, 0.0)).is_empty(),
        "MirrorUnion: mirror(0,0) should be empty"
    );
}
...
fn test_cpp_cross_section_decompose() {
    ...
    assert_eq!(
        decomp.len(),
        2,
        "Decompose: expected 2 pieces, got {}",
        decomp.len()
    );
    assert_eq!(
        decomp[0].num_contour(),
        2,
        "Decompose[0]: expected 2 contours, got {}",
        decomp[0].num_contour()
    );
    assert_eq!(
        decomp[1].num_contour(),
        2,
        "Decompose[1]: expected 2 contours, got {}",
        decomp[1].num_contour()
    );
    ...
    let vol_recomp =
        Manifold::extrude(&recomp.to_polygons(), 1.0, 0, 0.0, Vec2::new(1.0, 1.0)).volume();
    assert!(
        (vol_ab - vol_recomp).abs() < 1e-4,
        "Decompose: recomposed volume={} vs original={}",
        vol_recomp,
        vol_ab
    );
}
...
fn test_cpp_cross_section_hull() {
    ...
    assert!(
        (circ_area - annulus_hull.area()).abs() < 1.0,
        "Hull: annulus hull area != circ area"
    );
    // batch union of circs minus triangle center hull, area = circ_area * 2.5
    let batch_area = CrossSection::batch_boolean(&circs, OpType::Add)
        .difference(&tri)
        .area();
    assert!(
        (batch_area - circ_area * 2.5).abs() < 1.0,
        "Hull: batch-minus-tri area={} expected~{}",
        batch_area,
        circ_area * 2.5
    );
}
