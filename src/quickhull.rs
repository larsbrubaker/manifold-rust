...
impl Pool {
    ...
    fn reclaim(&mut self, v: Vec<usize>) {
        // Only reclaim if not excessively large
        if (v.len() + 1) * 128 < v.capacity() {
            return; // drop it to save memory
        }
        self.data.push(v);
    }
...
struct QuickHull {
    ...
    /// The vertex data used during construction. Set once from original_vertex_data,
    /// possibly extended with an extra planar point. Stored separately so we can
    /// read vertices while mutating mesh and other fields.
    verts: Vec<Vec3>,
...
impl QuickHull {
    ...
    fn build_mesh(mut self, eps: f64) -> (Vec<Halfedge>, Vec<Vec3>) {
