...
fn test_cube_has_vert_normals() {
    ...
    eprintln!(
        "Cube has {} vert_normals for {} verts",
        a.vert_normal.len(),
        a.num_vert()
    );
    ...
    assert_eq!(
        a.vert_normal.len(),
        a.num_vert(),
        "vert_normal should be populated"
    );
}
    ...
    for &offset in &[0.0, 0.1, 0.3, 0.5, 0.7, 0.9, 1.0, 1.5] {
        ...
        match result {
            Ok(r) => {
                ...
                eprintln!(
                    "Intersect offset={}: vol={:.4} verts={} tris={} empty={}",
                    offset,
                    vol,
                    r.num_vert(),
                    r.num_tri(),
...
fn test_boolean_mirrored() {
    ...
    assert!(
        (result.volume() - 0.75).abs() < 1e-5,
        "Volume should be 0.75, got {}",
        result.volume()
    );
    assert!(
        (result.surface_area() - 5.5).abs() < 1e-5,
        "Surface area should be 5.5, got {}",
        result.surface_area()
    );
...
fn test_boolean_volumes() {
    ...
    assert!(
        (m1.intersection(&m2).volume()).abs() < 1e-5,
        "m1^m2 should be 0"
    );
    assert!(
        (m1.union(&m2).union(&m4).volume() - 7.0).abs() < 1e-5,
        "m1+m2+m4 should be 7"
    );
    assert!(
        (m1.union(&m2).difference(&m4).volume() - 3.0).abs() < 1e-5,
        "m1+m2-m4 should be 3"
    );
    assert!(
        (m1.union(&m2.intersection(&m4)).volume() - 1.0).abs() < 1e-5,
        "m1+(m2^m4) should be 1"
    );
    assert!(
        (m7.intersection(&m4).volume() - 4.0).abs() < 1e-5,
        "m7^m4 should be 4"
    );
    assert!(
        (m7.intersection(&m3).intersection(&m1).volume() - 1.0).abs() < 1e-5,
        "m7^m3^m1 should be 1"
    );
    assert!(
        (m7.intersection(&m1.union(&m2)).volume() - 3.0).abs() < 1e-5,
        "m7^(m1+m2) should be 3"
    );
    assert!(
        (m7.difference(&m4).volume() - 3.0).abs() < 1e-5,
        "m7-m4 should be 3"
    );
    assert!(
        (m7.difference(&m4).difference(&m2).volume() - 1.0).abs() < 1e-5,
        "m7-m4-m2 should be 1"
    );
    assert!(
        (m7.difference(&m7.difference(&m1)).volume() - 1.0).abs() < 1e-5,
        "m7-(m7-m1) should be 1"
    );
    assert!(
        (m7.difference(&m1.union(&m2)).volume() - 4.0).abs() < 1e-5,
        "m7-(m1+m2) should be 4"
    );
}
...
fn test_boolean_almost_coplanar() {
    ...
    assert!(
        result.num_vert() >= 20 && result.num_vert() <= 22,
        "Expected ~20 verts, got {}",
        result.num_vert()
    );
    assert!(
        result.num_tri() >= 36 && result.num_tri() <= 40,
        "Expected ~36 tris, got {}",
        result.num_tri()
    );
...
fn test_boolean_perturb1() {
    ...
    // Small diamond
    let little_polys: Polygons = vec![vec![
        Vec2::new(2.0, 1.0),
        Vec2::new(3.0, 2.0),
        Vec2::new(2.0, 3.0),
        Vec2::new(1.0, 2.0),
    ...
    // Small triangle
    let punch_polys: Polygons = vec![vec![
        Vec2::new(1.0, 2.0),
        Vec2::new(2.0, 2.0),
        Vec2::new(2.0, 3.0),
    ]];
...
fn test_boolean_precision2() {
    ...
    assert!(
        ...
        "Near-precision intersection volume should be tiny: {}",
        intersection.volume()
    );
}
...
fn test_boolean_union_difference_stacked() {
    ...
    assert!(
        (result.volume() - blocksize * 2.0).abs() < 0.0001,
        "Stacked union volume: {} expected: {}",
        result.volume(),
        blocksize * 2.0
    );
}
